...
class UpFrontRocketBlockMaintenance {
	...
	function elements_mapping( $elements ) {
		...
		$default_skin = defined( 'upfront_DEFAULT_SKIN' ) ? upfront_DEFAULT_SKIN : 'base';
		$active_skin = UpFrontOption::get( 'current-skin', 'general', $default_skin );
		...
		if ( !isset( $this->option[$task][$active_skin] ) ) {
			...
			if ( version_compare( UPFRONT_VERSION, '1.0.0', '>=' ) ) {
			    ...
			    if ( $blocks ) {
				    ...
				    $all_elements = UpFrontElementsData::get_all_elements();
					...
					foreach ( $block_instance_elements as $element ) {
					    ...
					    /* we map the element properties to the correct UpFront instances */
					    if ( isset( $all_elements[$element]['properties'] ) )
						    foreach ( $all_elements[$element]['properties'] as $property => $property_value )
						        UpFrontElementsData::set_special_element_property( 'blocks', $element_with_no_instance, 'instance', $instance_to_register, $property, $property_value );
