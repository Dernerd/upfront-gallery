...
class PUR_AdminModel {
	...
	function __construct() {
		...
		butler_load( UPFRONTROCKET_COMPONENTS_PATH . 'api/class' );
	...
	function admin_notices() {
		...
		if ( $action == 'request-error' )
			if ( butler_get( 'error-code' ) == 1 )
				$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'Whoops! It would seem that you are not connected to the internet. Please check your internet connection and try again.', 'UpFrontrocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";
			else
				$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'Whoops! Our update server seems to be offline. It’s likely that we’re doing maintainance on the server, so please try again later.', 'UpFrontrocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";

		if ( $action == 'component-activated' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been activated successfully.', 'UpFrontrocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-activation-error' )
			$notice = '<div id="message" class="error"><p>' . sprintf( __( 'There was an error activating <strong>%s</strong>, please try again.', 'UpFrontrocket' ), butler_get( 'activated-component' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-deactivated' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been deactivated successfully.', 'UpFrontrocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
						
		if ( $action == 'component-deactivation-error' )
			$notice = '<div id="message" class="error"><p>' . sprintf( __( 'There was an error deactivating <strong>%s</strong>, please try again.', 'UpFrontrocket' ), butler_get( 'activated-component' ) ) . '</p></div>' . "\n";
		
		if ( $action == 'component-downloaded' )
			$notice = '<div id="message" class="success updated fade"><p>' . sprintf( __( '<strong>%s</strong> has been downloaded successfully.', 'UpFrontrocket' ),  butler_get( 'name' ) ) . '</p></div>' . "\n";
			
		if ( $action == 'download-component-error' )
			$notice .= '<div id="message" class="error"><p>' . sprintf( __( 'There was an error downloading <strong>%s</strong>, please try again.', 'UpFrontrocket' ), butler_get( 'name' ) ) . '</p></div>' . "\n";
