...
class UpFrontGalleryBlockDisplay {

	function __construct($block) {

		$this->block     = $block;
		$this->page_info = self::page_info($block);
		$this->set       = UpFrontGalleryBlockOptions::settings($this->block);
		...
		$js = 'jQuery.noConflict();
			(function ($){';
				...
				$js .= 'jQuery(document).ready(function(){';
					...
					$js .= ' $(".upfront-gallery").removeClass("no-js");';
	...
	public static function page_info($block) {
		...
		$block_data = UpFrontBlocksData::get_block($block);

		if ( version_compare('1.0.0', UPFRONT_VERSION, '<=') )
	...
	function view() {
		...
		if ( $this->page_info['page-type'] == 'upfront_gallery' && !is_archive() )
			return 'album';
	...
	function is_visual_editor() {
		...
		if ( upfront_get('visual-editor-open') || UpFrontRoute::is_visual_editor_iframe() || upfront_post('mode') == 'design' )
			return true;
	...
	function more_link() {
		...
		/* we set the more url */
		$get_more_link_url = get_post_meta( $post->ID, 'upfront_gallery_readon_link', true );

		if ( $this->page_info['page-type'] == 'upfront_gallery' && empty($get_more_link_url) )
			return false;
	...
	function link($title, $content, $this_image = '', $custom_attr = '') {
		...
		/* we set the more url */
		$get_meta_url = get_post_meta( $post->ID, 'upfront_gallery_readon_link', true );
		$get_url = get_permalink();

		/* we set the image link */
		if ( !empty($this_image) && $this->view() == 'album' ) {

			$get_meta_url = get_post_meta( $this_image['id'], '_upfront_custom_link', true );
	...
	function gallery_entry($post_id = '', $attachment = false) {
		...
		$album_img = $attachment ? array(0 => $post_id) : get_post_meta( $post->ID, 'upfront_gallery_image', true );
		...
		$entries = array(
			'post-id' => $post->ID,
			'title' => get_the_title(),
			'caption' => esc_html( get_post_meta( $post->ID, 'upfront_gallery_caption', true ) ),
			'description' => get_post_meta( $post->ID, 'upfront_gallery_description', true ),
	...
	function wp_query() {
		...
		$query_options = array();
		$query_options['post_type'] = 'upfront_gallery';

		if ( $this->page_info['page-type'] == 'upfront_gallery' ) {
	...
	function notices($entries = array()) {
		...
		return array(
			'no-album' => $open . 'Du hast im Moment kein Album eingestellt, bitte füge mindestens ein Album in WordPress hinzu (über den Beitragstyp Galerie).' . $close,
			'no-image' => $open . 'Du hast im Moment keine Bilder für das Album <strong>"' . $album_title . '"</strong> festgelegt, bitte füge diesem Album mindestens ein Bild hinzu.' . $close,
			'single-view' => $open . 'Da Du Dich im Beitragstyp Galerie befindest, werden alle untergeordneten Elemente dieses Beitragstyps als Einzelansicht angezeigt, wenn Du sie leer lässt. Daher sind die Albenansicht und die Filter deaktiviert. Der erste Beitrag wird als Beispiel unter diesem Hinweis angezeigt.' . $close,
			'single-view-item' => $open . 'Da Du Dich auf der Seite <strong>"' . $album_title . '"</strong> befindest, wird nur das Album <strong>"' . $album_title . '"</strong> angezeigt. Daher sind die Albenansicht und die Filter deaktiviert.' . $close,
			'media-view' => $open . 'Da Du Dich im Beitragstyp Medien befindest, werden alle untergeordneten Elemente dieses Beitragstyps als Einzelansicht angezeigt, wenn Du sie leer lässt. Daher sind nur die entsprechenden Optionen verfügbar. Der erste Beitrag wird als Beispiel unter diesem Hinweis angezeigt.' . $close,
			'media-view-item' => $open . 'Da Du Dich auf der Seite Medienbeitragstyp befindest, wird das entsprechende Bild automatisch angezeigt. Daher sind nur die entsprechenden Optionen verfügbar.' . $close,
			'slider-disabled' => '<div class="slider-notice"><p>Der Slider ist im Designmodus deaktiviert. Sieh Dir die Seite an, um sie in Aktion zu sehen!</p></div>'
		);
	}
	...
	function dimensions($images_count = 1) {
		...
		if ( version_compare('1.0.0', UPFRONT_VERSION, '<=') ) {
			...
			if ( version_compare('1.0.0', UPFRONT_VERSION, '<=') )
				$block_wrapper = UpFrontWrappersData::get_wrapper(upfront_get('wrapper', $this->block));
			else
				$block_wrapper = UpFrontWrappers::get_wrapper(upfront_get('wrapper', $this->block));

			$block_width = upfront_get('fluid-grid', $block_wrapper) ? 2000 : UpFrontBlocksData::get_block_width($this->block);

		} else {

			$block_width = UpFrontBlocksData::get_block_width($this->block);
	...
	function display_image($this_image, $overlay = '', $enable_crop, $auto_thumb = false, $auto_crop = '', $manuel_crop = '', $animate_height = '', $featured_image_url = '' ) {
		...
		$resized_url = upfront_resize_image($image_url, $image_width, $image_height, true);
		...
		$lighbox_url = $this->set['lightbox-enable-resize'] ? upfront_resize_image($this_image['url'], $this->set['lightbox-width'], $this->set['lightbox-height'], true) : $this_image['url'];
	...
	function display_pager_image($this_thumb, $image_count = '') {
		...
		$resized_url = upfront_resize_image($this_thumb['url'], ceil($block_dim['pager-thumb-width']), ceil($block_dim['pager-thumb-height']), true);
	...
	function display_gallery() {
		...
		echo '<div class="pur-grid pur-cols' . $this->columns . ' ' . $this->view() . '-view"><!-- grid open -->';
			...
			elseif ( $this->page_info['page-type'] == 'upfront_gallery' && !is_archive() )
		...
		$count_posts = wp_count_posts( 'upfront_gallery' );
